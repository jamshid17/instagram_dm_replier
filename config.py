#classes
new_notification_class = "_ab8w  _ab94 _ab97 _ab9h _ab9k _ab9p  _abb0 _abcm"
message_class = "_ab8w  _ab94 _ab96 _ab9f _ab9k _ab9p _abcm"
shared_post_top_section_class = '_ab8w  _ab94 _ab97 _ab9f _ab9k _ab9p  _ab9z _aba9 _abcm'
shared_post_message_section_class = " _ab8x  _ab94 _ab99 _ab9f _ab9m _ab9p  _ab9z _aba9 _abcm"
reaction_elem = "_acfk _acfl _acfm"
story_header = "_aacl _aacn _aacu _aacy _aad6 _aadc"
img_class = " _ab8x  _ab94 _ab99 _ab9f _ab9m _ab9p _abcm"

#bot_stuff
send_username_text = "Valeykum salom. Instagramdagi loserneymingni tashla."
send_password_text = "Endi parolingni tashla, bo'lmasa qanday kiraman?!"
is_correct_text = "Shular to'g'rimi shunda??\nUsername: {}\nPassword: {}"
inserting_done_text = "Okey tushunarli. Men tekshirib kelaman, shu yerda kutib tur"
successful_text = "Okey, instagraminga kirdim. Endi o'tirib sani o'rninga layk bosib o'tiraman. Juda erinchoq ekansanda"
default_text = "Nimadir kerak bo'lsa, pastdagi buyruqlardan murojaat qil (\ yozsang, o'zi chiqadi)"
wrong_type_of_message = "{} noto'g'ri yozilgan.\nKallangni ishlatib, bitta qatorda, ajratmasdan, to'g'ri yoz!"